...
If you have MSSQL Server Management Studio already installed and you have experience using it, then skip to the following section [Creating the database](#Creating%20the%20database).

###### Installing mssql server connection with Microsoft SQL executable tools
If your system has limited permissions then you can try the [docker setup](#Creating%20the%20mssql%20server%20connection%20with%20Docker) solution explained below and skip this section.
...
If you opted for creating the database via Microsoft SQL Server Management Studio and it went successfull then skip to the next section [Creating the database](#Creating%20the%20database).
---
If you have an older computer or dont have Windows 10 its better to skip this docker setup since various complicated issues may appear during installation and rely to [MSSQL instalation](#Installing%20mssql%20server%20connection%20with%20Microsoft%20SQL%20executable%20tools) without docker which is explained in the section above.
...
If any issues appear during installation troubleshoot them or skip to [MSSQL instalation](#Installing%20mssql%20server%20connection%20with%20Microsoft%20SQL%20executable%20tools) without docker.
